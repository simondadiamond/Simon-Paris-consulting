...
const SignupForm: React.FC<SignupFormProps> = ({ lang }) => {
  ...
  useEffect(() => {
    // Load Brevo's main.js *after* form is mounted
      ...
      <div id="sib-container" className="sib-container--large sib-container--vertical">
        ...
        >
          ...
          <div className="sib-input sib-form-block">
            <label htmlFor="EMAIL" className="block text-sm font-semibold text-[#121C2D]">
              {copy.emailLabel}
            </label>
            <input
              id="EMAIL"
              ...
              className="w-full rounded-lg border border-gray-300 px-4 py-2 text-[#121C2D]"
            />
            <label className="entry__error entry__error--primary hidden" htmlFor="EMAIL"></label>
          </div>
          ...
          <div className="sib-optin sib-form-block" data-required="true">
            <label className="flex items-start gap-2 text-sm text-[#4B5563]">
              <input
                ...
                value="1"
                required
                className="h-4 w-4 border-gray-300 text-[#139E9C] focus:ring-[#139E9C]"
              />
              {copy.consent}
            </label>
            <p className="text-xs text-[#6B7280]">{copy.specification}</p>
            <label className="entry__error entry__error--primary hidden" htmlFor="OPT_IN"></label>
          </div>
